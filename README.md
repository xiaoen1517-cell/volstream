...
**Binance WebSocket 在中国大陆通常无法直连**。如果你遇到类似下面的超时日志：

```
WebSocket 连接超时: timed out during opening handshake
```

项目默认已切换到 **OKX** 交易所（`config.yaml` 中 `exchange.name: okx`）。OKX WebSocket 在中国大陆通常比 Binance 稳定。

如需切换回 Binance，修改 `config.yaml`：
...
但切换回 Binance 后在中国大陆大概率需要代理。

### 配置代理

如果你有 SOCKS5/HTTP 代理，在 `.env` 中配置：
